...
import { getReservation, findReservationByReference, PROPERTY_IDS } from "../cloudbeds";
...
export async function handleUpdateGuest(req, res) {
    ...
    if (!matches || matches.length === 0) {
        // ✨ ON-DEMAND FALLBACK: Try Cloudbeds API directly if not in our index across all properties
        console.log(`[update_guest] No local match for ${resNorm}, trying Cloudbeds across ${PROPERTY_IDS.length} properties...`);

        for (const propertyID of PROPERTY_IDS) {
            console.log(`[update_guest] Searching property ${propertyID}...`);
            try {
                let cbData = null;
                try {
                    cbData = await getReservation(bookingValue, propertyID);
                } catch (innerErr) {
                    console.log(`[update_guest] getReservation failed for ${bookingValue} on prop ${propertyID}, trying Reference ID search...`);
                    cbData = await findReservationByReference(bookingValue, propertyID);
                }

                if (cbData) {
                    const cbResId = cbData.reservationId || cbData.reservationID;
                    console.log(`[update_guest] Cloudbeds match found in property ${propertyID}! Syncing ${cbResId}`);
                    ...
                    const requestNameParts = guestNameNorm.split(" ");
                    const firstName = requestNameParts[0];
                    const lastName = requestNameParts[requestNameParts.length - 1];

                    const nameMatches = cbGuestNameNorm === guestNameNorm ||
                        (requestNameParts.length >= 1 && cbGuestNameNorm.includes(firstName)) ||
                        (requestNameParts.length >= 2 && cbGuestNameNorm.includes(lastName));
                    ...
                    if (nameMatches) {
                        const { data: synced, error: syncErr } = await supabase
                            ...
                            .upsert({
                                ...
                                confirmation_number_raw: String(cbResId),
                                confirmation_number_norm: normalizeReservationNumber(cbResId),
                        ...
                        if (!syncErr && synced) {
                            matches = [synced];
                            break; // Stop searching other properties if found
                        }
                    } else {
                ...
                console.error(`[update_guest] Cloudbeds fallback failed for prop ${propertyID}:`, cbErr.message);
            }
        }
    }
