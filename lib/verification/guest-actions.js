...
import { getReservation, findReservationByReference } from "../cloudbeds";
...
export async function handleUpdateGuest(req, res) {
    ...
    if (!matches || matches.length === 0) {
        ...
        try {
            let cbData = null;
            try {
                cbData = await getReservation(bookingValue);
            } catch (innerErr) {
                console.log(`[update_guest] getReservation failed for ${bookingValue}, trying Reference ID search...`);
                cbData = await findReservationByReference(bookingValue);
            }

            if (cbData) {
                const cbResId = cbData.reservationId || cbData.reservationID;
                console.log(`[update_guest] Cloudbeds match found! Syncing ${cbResId}`);
